...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Test..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Test</h1>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 text-center space-x-3">
                ...
                <button @click="viewDetails(item.id)" title="View" class="text-green-500 hover:text-green-700"><i class="fas fa-eye"></i></button>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Test #{{ index + 1 }}</h2>
          <div class="flex gap-3 text-sm">
            <button @click="takeTest(item)" title="Take Test" class="text-purple-500 hover:text-purple-700"><i class="fas fa-play-circle"></i></button>
