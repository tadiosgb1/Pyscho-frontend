...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Progress</h1>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Progress #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Taken_at:</span>
              {{ item.taken_at }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-progress v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-progress v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddProgress, EditProgress, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/progress', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Progress-detail', params: { id } });
    },
    ...
    async confirmDelete() {
      ...
      if(res) {
        this.$root.$refs.toast.showToast('Progress deleted successfully', 'success');
      }
