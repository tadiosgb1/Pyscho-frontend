...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Schedules..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Schedules</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Identifier</th><th class="px-6 py-3 text-left">SenderName</th><th class="px-6 py-3 text-left">CompaignName</th><th class="px-6 py-3 text-left">Content</th><th class="px-6 py-3 text-left">AltContent</th><th class="px-6 py-3 text-left">To</th><th class="px-6 py-3 text-left">Tags</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.Identifier }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.senderName }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.compaignName }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.content }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.altContent }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.to }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.tags }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Schedules #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Identifier:</span>
              {{ item.Identifier }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">SenderName:</span>
              {{ item.senderName }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">CompaignName:</span>
              {{ item.compaignName }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">AltContent:</span>
              {{ item.altContent }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-schedules v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-schedules v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete Schedules"
      message="Are you sure you want to delete this Schedules?"
...
<script>
import AddSchedules from "./AddSchedules.vue";
import EditSchedules from "./EditSchedules.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddSchedules, EditSchedules, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/schedules', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Schedules-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/schedules', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Schedules deleted successfully', 'success');
      }
