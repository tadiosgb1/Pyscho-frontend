...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">SendeNames Detail</h1>
    </div>

    <!-- Detail Card -->
    <div class="bg-white overflow-hidden rounded-md border border-gray-200 p-4 hidden md:block space-y-2">
      <div><strong>ID:</strong> {{ item.id }}</div>
      <div><strong>Name:</strong> {{ item.name }}</div><div><strong>Status:</strong> {{ item.status }}</div>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      const response = await this.$apiGetById('/sendenames', id);
      this.item = response || {};
    } catch (error) {
      console.error(error);
    } finally {
