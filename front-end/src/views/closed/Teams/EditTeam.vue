    ...
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-pop-in border border-slate-100">
      ...
      <div class="p-8">
        <div class="flex justify-between items-start mb-6">
          <div>
            <h2 class="text-2xl font-black text-slate-900 leading-none">Edit Team</h2>
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-primary mt-2 flex items-center gap-2">
              <i class="fas fa-edit"></i> Modify Squad Parameters
            </p>
          </div>
          ...
          <div class="space-y-1">
            ...
            <div class="relative group">
              ...
              <textarea 
                ...
                required 
                rows="3"
                class="w-full pl-11 pr-4 py-3 bg-slate-50 border-2 border-slate-50 rounded-2xl focus:bg-white focus:ring-0 focus:border-primary outline-none transition-all text-sm font-bold text-slate-800 resize-none"
              ></textarea>
            </div>
          </div>
          ...
          <div class="flex gap-3 pt-4">
            ...
            >
              <template v-if="!loading">
                Update Squad <i class="fas fa-check-circle text-[8px] group-hover:scale-125 transition-transform"></i>
              </template>
              <template v-else>
...
export default {
  ...
  methods: {
    async submitForm() {
      this.loading = true;
      try {
        // Explicitly using PUT for edit
        const res = await this.$apiPut("/teams", this.data.id, this.form);
        if (res) {
          this.$root.$refs.toast.showToast('Team updated successfully', 'success');
          this.$emit("saved");
          this.$emit("close");
        }
      } catch (e) {
