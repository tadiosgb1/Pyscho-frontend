...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Option..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Option</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Text</th><th class="px-6 py-3 text-left">Question_id</th><th class="px-6 py-3 text-left">Value</th><th class="px-6 py-3 text-left">Weight</th><th class="px-6 py-3 text-left">Position</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.text }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.question_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.value }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.weight }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.position }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Option #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Text:</span>
              {{ item.text }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Question_id:</span>
              {{ item.question_id }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-option v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-option v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddOption, EditOption, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/option', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Option-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/option', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Option deleted successfully', 'success');
      }
