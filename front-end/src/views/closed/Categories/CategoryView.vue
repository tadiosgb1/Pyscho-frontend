...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Category..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Category</h1>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Category #{{ index + 1 }}</h2>
    ...
    <!-- Add/Edit Modal -->
    <add-category v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddCategory, EditCategory, Loading, DeleteConfirmModal },
  ...
  methods: {
    ...
    async confirmDelete() {
      ...
      if(res) {
        this.$root.$refs.toast.showToast('Category deleted successfully', 'success');
      }
