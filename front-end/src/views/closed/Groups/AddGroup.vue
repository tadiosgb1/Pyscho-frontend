    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          ...
          <input v-model="form.organization" type="text" required class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm transition duration-150" />
        </div>
    ...
    async submitForm() {
      try {
        ...
        if(res){
         ...
         const res= await this.$apiPut("/group",this.data.id ,this.form);
         if(res){
