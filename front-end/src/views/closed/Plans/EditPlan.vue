  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 text-sm overflow-y-auto max-h-screen">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block mb-1 text-sm font-medium text-gray-700">Max Users</label>
        ...
        <div class="flex items-center gap-2">
          <input type="checkbox" v-model="form.is_active" id="is_active" class="w-4 h-4 accent-green-500" />
          <label for="is_active" class="text-sm text-gray-700">Active (visible on pricing page)</label>
        </div>
...
export default {
  ...
  methods: {
    async submitForm() {
      this.loading = true;
      try {
        await this.$apiPut('/plans', this.data.id, this.form);
        this.$root.$refs.toast.showToast('Plan updated successfully', 'success');
        this.$emit('saved'); this.$emit('close');
      } catch (e) {
        console.error(e);
        this.$root.$refs.toast.showToast('Failed to update plan', 'error');
