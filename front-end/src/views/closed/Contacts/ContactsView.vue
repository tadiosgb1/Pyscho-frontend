...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Phone</th><th class="px-6 py-3 text-left">First_name</th><th class="px-6 py-3 text-left">Middle_name</th><th class="px-6 py-3 text-left">Last_name</th><th class="px-6 py-3 text-left">Title</th><th class="px-6 py-3 text-left">Age</th><th class="px-6 py-3 text-left">Gender</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.phone }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.first_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.middle_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.last_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.age }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.gender }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Contacts #{{ index + 1 }}</h2>
    ...
    <!-- Add/Edit Modal -->
    <add-contacts v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-contacts v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddContacts, EditContacts, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/contacts', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Contacts-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/contacts', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Contacts deleted successfully', 'success');
      }
