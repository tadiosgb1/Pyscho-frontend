...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Message</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Type</th><th class="px-6 py-3 text-left">To</th><th class="px-6 py-3 text-left">Message</th><th class="px-6 py-3 text-left">Parts</th><th class="px-6 py-3 text-left">Count</th><th class="px-6 py-3 text-left">Cost</th><th class="px-6 py-3 text-left">Sent_on</th><th class="px-6 py-3 text-left">Status</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.type }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.to }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.message }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.parts }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.count }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.cost }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.sent_on }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.status }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Message #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Parts:</span>
              {{ item.parts }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Count:</span>
              {{ item.count }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Sent_on:</span>
              {{ item.sent_on }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-message v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-message v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddMessage, EditMessage, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/message', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Message-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/message', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Message deleted successfully', 'success');
      }
